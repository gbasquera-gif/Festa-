import { useMemo } from "react";
import { useQuery } from "@tanstack/react-query";
import { EVENT_TYPES, eventTypeLabel } from "@festae/shared";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Checkbox } from "@/components/ui/checkbox";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Textarea } from "@/components/ui/textarea";
import { api } from "@/lib/api";

export interface Kit {
  id: string;
  name: string;
  basePrice: string | number;
  products: { productId: string; quantity: number; product: { id: string; name: string } }[];
}
export interface Tema {
  id: string;
  name: string;
}
export interface Produto {
  id: string;
  name: string;
  unitPrice: string | number;
  stockQuantity: number;
}

export interface ConflitoDetalhado {
  produto: string;
  estoqueTotal: number;
  jaComprometido: number;
  necessario: number;
  disponivel: number;
  reservasEmChoque: { id: string; cliente: string; status: string }[];
}

/**
 * Tudo que descreve uma festa vendida, em campos de texto.
 *
 * Guardado como string, e não como número, porque é o que o `<input>`
 * devolve: converter na hora de digitar transformaria "1," em 1 e apagaria a
 * vírgula debaixo do dedo de quem está digitando 1,50.
 */
export interface DadosDaReserva {
  nome: string;
  telefone: string;
  whatsapp: string;
  email: string;

  data: string;
  tipo: string;
  themeId: string;
  convidados: string;
  observacoes: string;

  kitId: string;
  extras: Record<string, number>;

  fulfillment: "PICKUP" | "DELIVERY";
  assembly: boolean;
  endereco: string;
  bairro: string;
  cidade: string;

  valorProdutos: string;
  entrega: string;
  montagem: string;
  desconto: string;
}

export const reservaEmBranco: DadosDaReserva = {
  nome: "",
  telefone: "",
  whatsapp: "",
  email: "",
  data: "",
  tipo: "ANIVERSARIO",
  themeId: "",
  convidados: "",
  observacoes: "",
  kitId: "",
  extras: {},
  fulfillment: "PICKUP",
  assembly: false,
  endereco: "",
  bairro: "",
  cidade: "Chapecó",
  valorProdutos: "",
  entrega: "0",
  montagem: "0",
  desconto: "0",
};

export const brl = (v: number) =>
  v.toLocaleString("pt-BR", { style: "currency", currency: "BRL" });

export const hojeISO = () => new Date().toISOString().slice(0, 10);

export const selectClass =
  "h-11 w-full rounded-md border border-input bg-transparent px-3 text-sm shadow-xs sm:h-9";

export function Campo({
  label,
  children,
  dica,
}: {
  label: string;
  children: React.ReactNode;
  dica?: string;
}) {
  return (
    <div className="flex flex-col gap-1.5">
      <Label>{label}</Label>
      {children}
      {dica && <p className="text-xs text-muted-foreground">{dica}</p>}
    </div>
  );
}

/** As listas que os dois formulários consultam. Cache compartilhado. */
export function useCatalogo() {
  const { data: kits } = useQuery({ queryKey: ["kits"], queryFn: () => api<Kit[]>("/kits") });
  const { data: temas } = useQuery({ queryKey: ["themes"], queryFn: () => api<Tema[]>("/themes") });
  const { data: produtos } = useQuery({
    queryKey: ["products"],
    queryFn: () => api<Produto[]>("/products"),
  });
  return { kits, temas, produtos };
}

/** O corpo da requisição, na forma que o servidor valida. */
export function corpoDaReserva(v: DadosDaReserva) {
  return {
    cliente: {
      nome: v.nome,
      telefone: v.telefone,
      whatsapp: v.whatsapp || undefined,
      email: v.email || undefined,
    },
    evento: {
      data: v.data,
      tipo: v.tipo,
      themeId: v.themeId || undefined,
      guestCount: v.convidados ? Number(v.convidados) : undefined,
      observacoes: v.observacoes || undefined,
    },
    produtos: {
      kitId: v.kitId || undefined,
      itens: Object.entries(v.extras)
        .filter(([, q]) => q > 0)
        .map(([productId, quantity]) => ({ productId, quantity })),
    },
    logistica: {
      fulfillment: v.fulfillment,
      assembly: v.assembly,
      endereco: v.endereco || undefined,
      bairro: v.bairro || undefined,
      cidade: v.cidade || undefined,
    },
  };
}

export function PainelDeConflitos({
  conflitos,
  titulo,
}: {
  conflitos: ConflitoDetalhado[];
  titulo: string;
}) {
  return (
    <Card className="mb-6 border-red-300 bg-red-50">
      <CardHeader className="pb-2">
        <CardTitle className="text-base text-red-800">{titulo}</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3 text-sm">
        {conflitos.map((c) => (
          <div key={c.produto}>
            <p className="font-semibold text-red-900">{c.produto}</p>
            <p className="text-red-800">
              Acervo {c.estoqueTotal} · já comprometido {c.jaComprometido} · disponível{" "}
              {c.disponivel} · esta reserva precisa de {c.necessario}
            </p>
            {c.reservasEmChoque.length > 0 && (
              <p className="text-red-700">
                Em choque com: {c.reservasEmChoque.map((r) => r.cliente).join(", ")}
              </p>
            )}
          </div>
        ))}
        <p className="text-red-800">
          Escolha outra data, reduza a quantidade ou remarque a outra festa.
        </p>
      </CardContent>
    </Card>
  );
}

/**
 * Os blocos que descrevem a festa: cliente, festa, produtos, logística e os
 * valores do pedido.
 *
 * Um componente só para registrar e para editar, de propósito. Dois
 * formulários com os mesmos campos escritos duas vezes divergem na primeira
 * mudança de regra — e a divergência aparece como a edição que apaga um campo
 * que o cadastro grava.
 *
 * O que fica de fora daqui é o que muda entre as duas telas: sinal recebido
 * (só existe no cadastro; na edição, pagamento é registro contábil) e origem
 * da venda (obrigatória ao nascer, apenas ajustável depois).
 */
export function BlocosDaReserva({
  valor,
  mudar,
}: {
  valor: DadosDaReserva;
  mudar: (parcial: Partial<DadosDaReserva>) => void;
}) {
  const { kits, temas, produtos } = useCatalogo();
  const kitEscolhido = kits?.find((k) => k.id === valor.kitId);
  const dataNoPassado = valor.data !== "" && valor.data < hojeISO();

  const total = useMemo(
    () =>
      Math.max(
        0,
        (Number(valor.valorProdutos) || 0) +
          (Number(valor.entrega) || 0) +
          (Number(valor.montagem) || 0) -
          (Number(valor.desconto) || 0),
      ),
    [valor.valorProdutos, valor.entrega, valor.montagem, valor.desconto],
  );

  return (
    <div className="space-y-6">
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">Cliente</CardTitle>
        </CardHeader>
        <CardContent className="grid gap-4 md:grid-cols-2">
          <Campo label="Nome *">
            <Input value={valor.nome} onChange={(e) => mudar({ nome: e.target.value })} required />
          </Campo>
          <Campo label="Telefone *" dica="É por ele que a operação confirma a festa no dia.">
            <Input
              value={valor.telefone}
              onChange={(e) => mudar({ telefone: e.target.value })}
              required
            />
          </Campo>
          <Campo label="WhatsApp">
            <Input
              value={valor.whatsapp}
              onChange={(e) => mudar({ whatsapp: e.target.value })}
              placeholder="se for diferente do telefone"
            />
          </Campo>
          <Campo label="E-mail" dica="Opcional. Quem fecha por WhatsApp costuma não passar.">
            <Input
              type="email"
              value={valor.email}
              onChange={(e) => mudar({ email: e.target.value })}
            />
          </Campo>
        </CardContent>
      </Card>

      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">Festa</CardTitle>
        </CardHeader>
        <CardContent className="grid gap-4 md:grid-cols-2">
          <Campo label="Data *" dica={dataNoPassado ? "Atenção: essa data já passou." : undefined}>
            <Input
              type="date"
              value={valor.data}
              onChange={(e) => mudar({ data: e.target.value })}
              required
              className={dataNoPassado ? "border-amber-500" : undefined}
            />
          </Campo>
          <Campo label="Tipo *">
            <select
              className={selectClass}
              value={valor.tipo}
              onChange={(e) => mudar({ tipo: e.target.value })}
            >
              {EVENT_TYPES.map((t) => (
                <option key={t} value={t}>
                  {eventTypeLabel(t)}
                </option>
              ))}
            </select>
          </Campo>
          <Campo label="Tema">
            <select
              className={selectClass}
              value={valor.themeId}
              onChange={(e) => mudar({ themeId: e.target.value })}
            >
              <option value="">Sem tema</option>
              {temas?.map((t) => (
                <option key={t.id} value={t.id}>
                  {t.name}
                </option>
              ))}
            </select>
          </Campo>
          <Campo label="Convidados">
            <Input
              type="number"
              inputMode="numeric"
              min={1}
              value={valor.convidados}
              onChange={(e) => mudar({ convidados: e.target.value })}
              placeholder="a combinar"
            />
          </Campo>
          <div className="md:col-span-2">
            <Campo label="Observações" dica="O que a equipe precisa saber no dia.">
              <Textarea
                rows={3}
                value={valor.observacoes}
                onChange={(e) => mudar({ observacoes: e.target.value })}
                placeholder="Ex.: bolo chega às 14h; portão azul; subir pelo elevador de serviço."
              />
            </Campo>
          </div>
        </CardContent>
      </Card>

      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">Produtos</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <Campo label="Kit">
            <select
              className={selectClass}
              value={valor.kitId}
              onChange={(e) => mudar({ kitId: e.target.value })}
            >
              <option value="">Sem kit (só itens avulsos)</option>
              {kits?.map((k) => (
                <option key={k.id} value={k.id}>
                  {k.name} — {brl(Number(k.basePrice))}
                </option>
              ))}
            </select>
          </Campo>

          {kitEscolhido && (
            <div className="rounded-md bg-muted/50 p-3 text-sm">
              <p className="mb-1 font-medium">Já vem no kit:</p>
              <p className="text-muted-foreground">
                {kitEscolhido.products.map((p) => `${p.quantity}× ${p.product.name}`).join(" · ")}
              </p>
            </div>
          )}

          <div>
            <Label className="mb-2 block">Itens adicionais</Label>
            <div className="max-h-64 space-y-1 overflow-y-auto rounded-md border p-3">
              {produtos?.map((p) => (
                <div key={p.id} className="flex items-center justify-between gap-3 py-1.5">
                  <span className="min-w-0 text-sm">
                    <span className="block truncate">{p.name}</span>
                    <span className="text-xs text-muted-foreground">
                      {brl(Number(p.unitPrice))} · acervo {p.stockQuantity}
                    </span>
                  </span>
                  {/* inputMode numérico abre o teclado de números no iPhone;
                      sem isso a pessoa digita quantidade num teclado de letras. */}
                  <Input
                    type="number"
                    inputMode="numeric"
                    min={0}
                    className="h-11 w-20 shrink-0 text-center sm:h-9"
                    value={valor.extras[p.id] ?? ""}
                    placeholder="0"
                    onChange={(e) =>
                      mudar({
                        extras: { ...valor.extras, [p.id]: Number(e.target.value) || 0 },
                      })
                    }
                  />
                </div>
              ))}
            </div>
          </div>
        </CardContent>
      </Card>

      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">Logística</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex flex-wrap gap-4">
            <label className="flex items-center gap-2 text-sm">
              <input
                type="radio"
                checked={valor.fulfillment === "PICKUP"}
                onChange={() => mudar({ fulfillment: "PICKUP" })}
              />
              Retirada na sede
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input
                type="radio"
                checked={valor.fulfillment === "DELIVERY"}
                onChange={() => mudar({ fulfillment: "DELIVERY" })}
              />
              Entrega no local
            </label>
            <label className="flex items-center gap-2 text-sm">
              {/* Montagem é escolha independente da logística: a equipe
                  monta no local mesmo quando a cliente buscou os itens. */}
              <Checkbox
                checked={valor.assembly}
                onCheckedChange={(v) => mudar({ assembly: v === true })}
              />
              Com montagem
            </label>
          </div>
          {valor.fulfillment === "DELIVERY" && (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <div className="md:col-span-2">
                <Campo label="Endereço">
                  <Input
                    value={valor.endereco}
                    onChange={(e) => mudar({ endereco: e.target.value })}
                  />
                </Campo>
              </div>
              <Campo label="Bairro">
                <Input value={valor.bairro} onChange={(e) => mudar({ bairro: e.target.value })} />
              </Campo>
              <Campo label="Cidade">
                <Input value={valor.cidade} onChange={(e) => mudar({ cidade: e.target.value })} />
              </Campo>
            </div>
          )}
        </CardContent>
      </Card>

      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">Valores</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Campo label="Valor dos produtos *">
              <Input
                type="number"
                inputMode="decimal"
                step="0.01"
                min={0}
                value={valor.valorProdutos}
                onChange={(e) => mudar({ valorProdutos: e.target.value })}
                required
              />
            </Campo>
            <Campo label="Entrega">
              <Input
                type="number"
                inputMode="decimal"
                step="0.01"
                min={0}
                value={valor.entrega}
                onChange={(e) => mudar({ entrega: e.target.value })}
              />
            </Campo>
            <Campo label="Montagem">
              <Input
                type="number"
                inputMode="decimal"
                step="0.01"
                min={0}
                value={valor.montagem}
                onChange={(e) => mudar({ montagem: e.target.value })}
              />
            </Campo>
            <Campo label="Desconto">
              <Input
                type="number"
                inputMode="decimal"
                step="0.01"
                min={0}
                value={valor.desconto}
                onChange={(e) => mudar({ desconto: e.target.value })}
              />
            </Campo>
          </div>

          {/* O valor é o que foi combinado com a cliente, não o da tabela:
              na venda por WhatsApp há negociação, e recalcular por cima
              faria o sistema discordar do que ela já ouviu. */}
          <div className="rounded-md bg-muted/50 p-3 text-sm">
            <p>
              Total do pedido: <strong>{brl(total)}</strong>
            </p>
          </div>
        </CardContent>
      </Card>
    </div>
  );
}
